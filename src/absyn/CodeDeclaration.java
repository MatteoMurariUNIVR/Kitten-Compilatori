    ...
    protected void translateReferenced(Block block, Set<ClassMemberSignature> done, Set<Block> blocksDone) {
    	...
    	for (BytecodeList cursor = block.getBytecode(); cursor != null; cursor = cursor.getTail()) {
    		...
    		if (h instanceof GETFIELD)
    		{
    			...
    			for(ConstructorSignature con : ((GETFIELD) h).getField().getDefiningClass().getConstructors())
    				con.getAbstractSyntax().translate(done);
    			for(FixtureSignature fs : ((GETFIELD) h).getField().getDefiningClass().getFixtures())
    				fs.getAbstractSyntax().translate(done);
    			for(TestSignature ts : ((GETFIELD) h).getField().getDefiningClass().getTests())
    				ts.getAbstractSyntax().translate(done);
    		}
    		else if (h instanceof PUTFIELD)
    		{
    			done.add(((PUTFIELD) h).getField());
    			for(ConstructorSignature con : ((PUTFIELD) h).getField().getDefiningClass().getConstructors())
    				con.getAbstractSyntax().translate(done);
    			for(FixtureSignature fs : ((PUTFIELD) h).getField().getDefiningClass().getFixtures())
    				fs.getAbstractSyntax().translate(done);
    			for(TestSignature ts : ((PUTFIELD) h).getField().getDefiningClass().getTests())
    				ts.getAbstractSyntax().translate(done);
    		}
    		else if (h instanceof CALL)
    		{
    			for (CodeSignature callee: ((CALL)h).getDynamicTargets())
    			{
    				...
    				for(ConstructorSignature con : callee.getDefiningClass().getConstructors())
    					con.getAbstractSyntax().translate(done);
    				for(FixtureSignature fs : callee.getDefiningClass().getFixtures())
    					fs.getAbstractSyntax().translate(done);
    				for(TestSignature ts : callee.getDefiningClass().getTests())
    					ts.getAbstractSyntax().translate(done);
    			}
    		}
    	}
